...
void simple_test(T i) {
    ...
    spacker::unpack_psip<spacker::Multiplier<>>(packed.size(), packed.data(), unpacked.size(), unpacked.data());
    EXPECT_EQ(sample, unpacked);
}

TEST(UnpackMultiplierTest, BasicByte) {
...
TEST(UnpackMultiplierTest, BasicShort) {
...
TEST(UnpackMultiplierTest, BasicWord) {
    // Trying some bigger, juicer numbers.
...
TEST(UnpackMultiplierTest, RleByte) {
    for (int i = 1; i <= std::numeric_limits<uint8_t>::max(); ++i) {
...
TEST(UnpackMultiplierTest, RleShort) {
    // Trying some bigger, juicer numbers.
    std::vector<uint16_t> targets{ 1, 10, 100, 1000, 10000 };
    for (auto t : targets) {
...
TEST(UnpackMultiplierTest, RleWord) {
    // Trying some bigger, juicer numbers.
    std::vector<uint32_t> targets{ 1, 10, 100, 1000, 10000, 100000, 1000000 };
    for (auto t : targets) {
...
void compare(const std::vector<T>& input) {
    auto packed = spacker::pack_psip<rle, spacker::Multiplier<>>(input.size(), input.data());
    std::vector<T> unpacked(input.size());
    spacker::unpack_psip<spacker::Multiplier<>>(packed.size(), packed.data(), unpacked.size(), unpacked.data());
    EXPECT_EQ(input, unpacked);
}

TEST(UnpackMultiplierTest, RandomUint8) {
...
TEST(UnpackMultiplierTest, RandomUint16) {
...
TEST(UnpackMultiplierTest, RandomUint32) {
...
TEST(UnpackMultiplierTest, RleUint8) {
...
TEST(UnpackMultiplierTest, RleUint16) {
...
TEST(UnpackMultiplierTest, RleUint32) {
